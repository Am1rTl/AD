#IMP inc/stdout
#IMP inc/bytelists

#EXT run_extern		...
#EXT get_env		1
#EXT atoi			1
#EXT read			2
#EXT mkdir			1
#EXT readfile		1

#EXT strstr			2
#EXT strcat			2
#EXT strlen			1
#EXT strcmp			2

#EXT writefile		2

#EXT readcmd		1
#EXT itoa			1
#EXT dircnt			1

isuservalid(,username,password){
	data = 0;
	dirstart = "/data/";
	passfile = securecat:(,dirstart,username);
	passfile = strcat:(,passfile,"/password");



	if(!strcmp:(,username,"admin")){
		res = 1;
		res = res & (($(password) & 0xff) == 35);
		res = res & (($(password+1) & 0xff) == 95);
		res = res & (($(password+2) & 0xff) == 43);
		res = res & (($(password+3) & 0xff) == 84);
		res = res & (($(password+4) & 0xff) == 104);
		res = res & (($(password+5) & 0xff) == 49);
		res = res & (($(password+6) & 0xff) == 53);
		res = res & (($(password+7) & 0xff) == 95);
		res = res & (($(password+8) & 0xff) == 80);
		res = res & (($(password+9) & 0xff) == 64);
		res = res & (($(password+10) & 0xff) == 115);
		res = res & (($(password+11) & 0xff) == 53);

		if(res){
			return 3;
		}
		return 1;
	}


	data = readfile:(,passfile);

	if(!data){
		return 0;
	}

	if(strcmp:(,data,password)){
		return 1;
	}
	return 2;
}

sendmsg(,username,senduser,message){
	dirstart 	= "/data/";
	cnt = 0;

	message		= strcat:(,message,"\n");

	fromdir 	= strcat:(,dirstart,username);
	fromdir		= strcat:(,fromdir,"/");
	fromdir		= strcat:(,fromdir,senduser);
	todir		= strcat:(,dirstart,senduser);

	tocmd		= strcat:(,"ls"," ");
	tocmd		= strcat:(,tocmd,"-la");
	tocmd		= strcat:(,tocmd," ");
	tocmd		= strcat:(,tocmd,"'");
	tocmd		= strcat:(,tocmd,todir);
	tocmd		= strcat:(,tocmd,"'");
	tocmd		= strcat:(,tocmd," ");
	tocmd		= strcat:(,tocmd,"2>/dev/null");

	todata		= readcmd:(,tocmd);
	if(!strlen:(,todata)){
		return 0;
	}

	todir		= strcat:(,todir,"/");
	todir		= strcat:(,todir,username);

	mkdir:(,fromdir);
	mkdir:(,todir);


	cnt = dircnt:(,todir);
	todir = strcat:(,todir,"/");
	todir = strcat:(,todir,itoa:(,cnt));

	fromdir = strcat:(,fromdir,"/");
	fromdir = strcat:(,fromdir,itoa:(,cnt));

	writefile:(,todir,message);

	message = strcat:(,"You> ", message);
	writefile:(,fromdir,message);

	return 1;
}

init()
{	
	userln = 0;
	passln = 0;
	username = 0;
	password = 0;
	val = 0;

	cont_leng = get_env:(,"CONTENT_LENGTH");
	if(cont_leng){
		content_l = atoi:(,cont_leng);
		data = bl_allc:(,0,content_l);
		read:(,data,content_l);
		
		userln = strlen:(,data);
		passln = strlen:(,data+userln+1);
		suserl = strlen:(,data+userln+passln+2);

		username = data;
		password = data+userln+1;
		senduser = password+passln+1;
		message	 = senduser+suserl+1;

		val = isuservalid:(,username,password);
		if(val == 0 || val == 1){
			prints:(,"BAD CRED");
		}else{
			if(!sendmsg:(,username,senduser,message)){
				prints:(,"MSG NOTSENT");
			}else{
				prints:(,"MSG SENT");
			}
		}
	}

	return 0;
}

