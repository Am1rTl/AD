...
void install_seccompx() {
  ...
  int syscall_blacklist[] = {SCMP_SYS(openat),SCMP_SYS(execve),SCMP_SYS(fork),SCMP_SYS(vfork),SCMP_SYS(prctl),SCMP_SYS(execveat)};
  ...
  for(int i = 0;i < len;i++) {
    if (seccomp_rule_add(ctx, SCMP_ACT_KILL_PROCESS, syscall_blacklist[i], 0) != 0) {
      perror("Failed to install filter");
      _exit(-1);
    }
  }
  if(seccomp_load(ctx)){
    perror("Failed to load seccomp.");
    _exit(-1);
  }
  return;
};
